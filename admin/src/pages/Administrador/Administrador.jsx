...
const Administrador = () => {
  ...
  useEffect(() => {
    const cargarDatos = async () => {
      try {
        const res = await axios.get("http://localhost:5000/api/pagos");
        setPagos(res.data);
      } catch (err) {
  ...
  const guardarPago = async () => {
    ...
    const datosEnvio = {
      cliente: nombre.toUpperCase(),
      monto: parseFloat(formData.monto),
      id_pago: editingPago ? editingPago.id_pago : Math.floor(1000 + Math.random() * 9000)
    };

    try {
      if (editingPago) {

        await axios.put(`http://localhost:5000/api/pagos/${editingPago._id}`, datosEnvio);
        showAlert("SISTEMA", "Cambios guardados en MongoDB.", "success");
      } else {

        await axios.post("http://localhost:5000/api/pagos", datosEnvio);
        showAlert("SISTEMA", "Cliente registrado en MongoDB.", "success");
      }
  ...
  const eliminarPago = async (mongoId) => {
    if (window.confirm("¿Estás seguro de eliminar este registro de MongoDB?")) {
      try {
        await axios.delete(`http://localhost:5000/api/pagos/${mongoId}`);
        // Quitamos el elemento de la pantalla sin recargar
        setPagos(pagos.filter(p => p._id !== mongoId));
        showAlert("SISTEMA", "Registro borrado con éxito", "success");
      } catch (error) {
      ...
      <div className="data-card">
        <table className="modern-table">
          ...
          <tbody>
            {filtrados.map((p) => (
              <tr key={p._id}>
                <td style={{ color: '#64748b', fontWeight: '600' }}>#ID-{p.id_pago}</td>
                ...
                <td>
                  <button className="dot-btn" onClick={() => setMenuOpcionesId(menuOpcionesId === p._id ? null : p._id)}><FaBars /></button>
                  {menuOpcionesId === p._id && (
                    <div className="action-menu">
                      <button onClick={() => { setEditingPago(p); setFormData({ cliente: p.cliente, monto: p.monto }); setShowModal(true); }}><FaEdit /> Editar</button>
                      <button onClick={() => eliminarPago(p._id)} style={{ color: '#ef4444' }}> <FaTrash /> Borrar </button>
                    </div>
                  )}
                </td>
              </tr>
